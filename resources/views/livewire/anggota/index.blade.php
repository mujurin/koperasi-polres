<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
            ...
            <flux:subheading>
                Daftar anggota yang tersinkronisasi dengan API SIAPKLU.
                @if(method_exists($anggotas, 'total'))
                    <span class="ml-1 font-semibold text-indigo-600 dark:text-indigo-400">
        ...
        <div class="flex items-center gap-2">
            <flux:input wire:model.live.debounce.300ms="search" icon="magnifying-glass"
                placeholder="Cari NIP atau Nama..." class="w-full sm:w-64" />
            ...
            <button wire:click="syncData" wire:loading.attr="disabled" wire:target="syncData"
                ...
                <flux:icon wire:loading.remove wire:target="syncData" name="arrow-path"
                    class="size-4 text-indigo-600 dark:text-indigo-400" />
                <svg wire:loading wire:target="syncData"
                    class="size-4 animate-spin text-indigo-600 dark:text-indigo-400" xmlns="http://www.w3.org/2000/svg"
    ...
    <div
        class="overflow-x-auto rounded-3xl border border-zinc-200 bg-white shadow-sm dark:border-zinc-700 dark:bg-zinc-950">
        <table class="min-w-full border-collapse text-left text-sm text-zinc-700 dark:text-zinc-200">
            <thead
                class="bg-zinc-50 text-xs uppercase tracking-[0.15em] text-zinc-500 dark:bg-zinc-900 dark:text-zinc-400">
                <tr>
                ...
                @forelse($anggotas as $anggota)
                    <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-900/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-right">
                            ...
                            @endphp
                            @if($totalPinjaman > 0)
                                <span class="font-bold text-emerald-600 dark:text-emerald-400">Rp
                                    {{ number_format($totalPinjaman, 0, ',', '.') }}</span>
                            @else
                                <span class="text-zinc-400">-</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-center">
                            @if($anggota->is_active)
                                <span
                                    class="inline-flex items-center rounded-md bg-emerald-50 px-2 py-1 text-xs font-medium text-emerald-700 ring-1 ring-inset ring-emerald-600/20 dark:bg-emerald-500/10 dark:text-emerald-400 dark:ring-emerald-500/20">Aktif</span>
                            @else
                                <span
                                    class="inline-flex items-center rounded-md bg-rose-50 px-2 py-1 text-xs font-medium text-rose-700 ring-1 ring-inset ring-rose-600/10 dark:bg-rose-400/10 dark:text-rose-400 dark:ring-rose-400/30">Nonaktif</span>
                            @endif
                        </td>
                        <td class="px-6 py-4 text-right">
                            <button wire:click="editAnggota({{ $anggota->id }})"
                                class="inline-flex items-center gap-1.5 rounded-lg border border-zinc-200 bg-white px-2.5 py-1.5 text-xs font-medium text-zinc-700 shadow-sm transition-colors hover:bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 dark:hover:bg-zinc-700">
                ...
                @empty
                    <tr>
                        <td colspan="6" class="px-6 py-12 text-center">
                            ...
                            <p class="text-zinc-400 dark:text-zinc-500 text-sm mt-1">Klik tombol "Sinkronkan Data API" di
                                atas untuk mulai memuat data.</p>
                        </td>
                    </tr>
                @endforelse
            </tbody>
    ...
    <flux:modal name="modal-edit-anggota" class="md:w-[28rem]">
        <form wire:submit="saveAnggota" class="space-y-6">
            ...
            <div class="space-y-4">
                <flux:switch wire:model="editingStatus" label="Status Aktif Anggota"
                    description="Jika dinonaktifkan, anggota tetap ada di database namun berstatus nonaktif." />
            </div>
